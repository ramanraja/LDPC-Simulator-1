/**
 * Peg algorithm for building LDPC codes
...
public class Peg extends Generator
{
	int colwt;
	...
	// the entry point for making all connections
	public void generate() throws Exception
	{
		...
		for (int c=0; c<cols; c++)
		{
			...
			connect (c, row);
			for (int w=1; w<colwt; w++) // the remaining edges
			{
				buildTree(c);
				//printDistances();
	...
	{
		int avgRowwt = (colwt*cols)/rows;
		return "Peg: " +rows +" rows; " +cols +" cols; colwt=" +colwt +"; avg rowwt=" +avgRowwt;
	}
